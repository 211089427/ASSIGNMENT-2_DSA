...
service /graphql on new graphql:Listener(8080) {
    ...
    //MUTATIONS
    remote function createUser(string firstName, string lastName, string jobTitle, string position, UserRole role, int departmentId, string token) returns User|error {
    ...
    remote function  updateUser(int id, string firstName, string lastName, string jobTitle, string position, UserRole role, int departmentId, string token) returns User|error {
            if (!isAuthenticated(token)) {
                return error("Authentication failed. Please log in.");
            }
    ...
    remote function  deleteUser(int id, string token) returns boolean|error {
            if (!isAuthenticated(token)) {
                return error("Authentication failed. Please log in.");
            }        
    ...
    remote function createDepartment(string name, string token) returns Department|error {
            if (!isAuthenticated(token)) {
                return error("Authentication failed. Please log in.");
            }        
    ...
    remote function updateDepartment(int id, string name, string token) returns Department|error {
            if (!isAuthenticated(token)) {
                return error("Authentication failed. Please log in.");
            }        
    ...
    remote function deleteDepartment(int id, string token) returns boolean|error {
            if (!isAuthenticated(token)) {
                return error("Authentication failed. Please log in.");
            }        
    ...
    remote function deleteDepartmentObjective(int id, string token) returns boolean|error {
    ...
    remote function createKPI(int userId, string name, string metric, string unit, string token) returns KPI|error {
            if (!isAuthenticated(token)) {
    ...
    remote function updateKPI(int id, int userId, string name, string metric, string unit, float score, string token) returns KPI|error {
            if (!isAuthenticated(token)) {
                return error("Authentication failed. Please log in.");
            }
    ...
    remote function  deleteKPI(int id, string token) returns boolean|error {
            if (!isAuthenticated(token)) {
                return error("Authentication failed. Please log in.");
            }
        ...
        if (response is sql:ExecutionResult) {
            ...
            return error("Failed to delete ID");
        }
    }
    ...
    remote function  gradeKPI(int supervisorId, int kpiId, float grade, string token) returns boolean|error {
            if (!isAuthenticated(token)) {
                return error("Authentication failed. Please log in.");
            }
    ...
    remote function  register(string firstName, string lastName, string email, string password, string jobTitle, string position, string role, int departmentId) returns string|error {
